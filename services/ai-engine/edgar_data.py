...
from observability import tracer, record_external_call
...
class EdgarDataProvider:
    ...
    def _fetch(self, url: str) -> Dict:
        ...
        with tracer.start_as_current_span("external_api.wait") as span:
            span.set_attribute("peer.service", "sec")
            span.set_attribute("external.operation", "fetch")
            response = requests.get(url, headers=SEC_HEADERS, timeout=15)
            response.raise_for_status()
            record_external_call((time.perf_counter() - start) * 1000.0, "sec", "fetch")
    ...
    def get_cik(self, ticker: str) -> Optional[str]:
        ...
        try:
            ...
            with tracer.start_as_current_span("external_api.wait") as span:
                span.set_attribute("peer.service", "sec")
                span.set_attribute("external.operation", "ticker_lookup")
                response = requests.get(tickers_url, headers=SEC_HEADERS, timeout=10)
                response.raise_for_status()
                data = response.json()
                record_external_call((time.perf_counter() - start) * 1000.0, "sec", "ticker_lookup")
